...
IncPBConstraint::IncPBConstraint(vector<WeightedLit> const & literals, Comparator comparator, int64_t less_eq, int64_t greater_eq) : leq(less_eq), geq(greater_eq), init_leq(leq), init_geq(geq),  weighted_literals(literals), comparator(comparator), isDualEncoded(false)
{
  assert(comparator == BOTH);
}
IncPBConstraint::IncPBConstraint(vector<WeightedLit> const & literals, Comparator comparator, int64_t bound) : leq(0), geq(0), init_leq(leq), init_geq(geq), weighted_literals(literals), comparator(comparator), isDualEncoded(false)
{
  assert(comparator != BOTH);
  if (comparator == LEQ)
  {
