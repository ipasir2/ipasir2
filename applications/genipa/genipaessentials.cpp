...
bool loadFormulaDualRailed(void* solver, const char* filename, int* outVariables) {
	...
	while (c != EOF) {
		...
		if (isdigit(c)) {
			...
			// add to the solver
			ipasir2_add(solver, getDualRailName(num));
		}
	}
	fclose(f);
	// add the extra clauses
	for (int var = 1; var <= maxVar; var++) {
		ipasir2_add(solver, -getDualRailName(var));
		ipasir2_add(solver, -getDualRailName(-var));
...
int main(int argc, char **argv) {
	...
	void *solver;
	ipasir2_init(&solver);
	ipasir2_set_learn(solver, NULL, learnCb);
	int variables = 0;
	...
	for (int var = 1; var <= variables; var++) {
		...
		ipasir2_assume(solver, -getDualRailName(var));
		ipasir2_assume(solver, -getDualRailName(-var));
		ipasir2_solve(solver, &satRes);
		if (satRes == 10) {
